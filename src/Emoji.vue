<template>
  <div>
    <Poptip placement="top-start" :transfer="true" v-model="pickerVisible">
      ...
      <div slot="content">
        <div class="picker">
          ...
          >
            <div v-for="d in item" class="item-box" @click="onSelect(d)">
              {{ getUnicode(d) }}
            </div>
          </div>
          <div class="toggle-box">
            <div
              ...
              :class="{ dot: true, active: selectPanel === index }"
            ></div>
          </div>
        </div>
      </div>
